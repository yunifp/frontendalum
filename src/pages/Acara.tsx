            ...
            <section className="relative pt-32 pb-20 lg:pt-40 lg:pb-32 border-b border-stone-300">
                ...
                <div className="max-w-4xl mx-auto px-6 text-center space-y-8 relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                    ...
                    <div className="flex items-center gap-3 justify-center mb-6">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#1e3a8a]">Kalender Akademik</span>
                        <div className="h-[1px] w-12 bg-[#1e3a8a]"></div>
                    </div>

                    <h1 className="text-5xl lg:text-7xl font-serif text-stone-900 leading-[1.1] tracking-tight">
                        Jadwal Acara & <br className="hidden sm:block" />
                        <span className="italic text-stone-600">Kegiatan Alumni</span>
                    </h1>
                    ...
                    <p className="text-stone-600 text-lg md:text-xl font-serif italic max-w-2xl mx-auto leading-relaxed border-x border-stone-300 px-6">
                        "Ikuti berbagai lokakarya, forum diskusi, dan reuni eksklusif. Jangan lewatkan momentum untuk merawat relasi dan berkembang bersama."
                    </p>
                    ...
                    {isLoading ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                            {[1, 2, 3, 4, 5, 6].map(i => (
                                <div key={i} className="flex flex-col border-t border-stone-300 pt-6">
                                    <div className="h-56 bg-stone-200 animate-pulse border border-stone-300 mb-6" />
                        ...
                        <div className="text-center py-32 bg-[#FAF9F6] border-y border-stone-300 relative">
                            ...
                            <div className="relative z-10 bg-[#FAF9F6] inline-block px-8">
                                <CalendarX size={40} className="mx-auto text-stone-300 mb-6" strokeWidth={1} />
                                <h3 className="text-3xl font-serif text-stone-900 tracking-tight mb-4">Lembaran Kosong</h3>
                                <p className="text-stone-500 font-serif italic max-w-sm mx-auto">Kalender akademik belum memuat agenda baru. Pantau terus halaman ini untuk pengumuman mendatang.</p>
                            </div>
                        </div>
                    ) : (
                            ...
                            {events.map((event) => {
                                ...
                                return (
                                    <Link
                                        to={`/acara/${event.slug}`}
                                        key={event.id_post}
                                        className="group flex flex-col pt-6 border-t-2 border-transparent hover:border-[#1e3a8a] transition-all duration-500 relative"
                                    >
                                        <div className="absolute top-0 left-0 right-0 h-[1px] bg-stone-300 group-hover:bg-transparent transition-colors"></div>
                                        <div className="h-56 w-full overflow-hidden relative bg-stone-200 border border-stone-300 mb-6">
                                            <SafeImage
                                                src={getPublicImageUrl(event.thumbnail)}
                                                alt={event.judul}
                                            ...
                                            <div className="absolute top-4 left-4 bg-[#FAF9F6] border border-stone-900 p-2 min-w-[3.5rem] flex flex-col items-center justify-center shadow-[2px_2px_0px_0px_rgba(28,25,23,1)]">
                                                <span className="text-[10px] font-bold text-[#1e3a8a] uppercase tracking-widest mb-1">
                                                    {format(eventDate, 'MMM', { locale: id })}
                                                </span>
                                                <span className="text-2xl font-serif text-stone-900 leading-none">
                                                    {format(eventDate, 'dd')}
                                                </span>
                                            </div>

                                            <div className="absolute bottom-4 right-4">
                                                <span className="bg-stone-900 text-[#FAF9F6] font-bold uppercase text-[9px] tracking-[0.2em] px-3 py-1.5 border border-stone-900">
                                                    Agenda
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex-1 flex flex-col">
                                            <h3 className="text-2xl font-serif font-medium text-stone-900 leading-snug line-clamp-2 group-hover:text-[#1e3a8a] transition-colors mb-6 flex-1">
                                                {event.judul}
                                            </h3>
                                            ...
                                            <div className="pt-4 border-t border-stone-200 mt-auto">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-stone-500 font-bold text-[10px] uppercase tracking-[0.2em] flex items-center gap-2">
                                                        <Ticket size={14} className="text-stone-400" /> Registrasi
                                                    </span>
                                                    <span className="flex items-center text-stone-900 font-bold text-[10px] uppercase tracking-[0.2em] gap-2 group-hover:text-[#1e3a8a] transition-colors">
